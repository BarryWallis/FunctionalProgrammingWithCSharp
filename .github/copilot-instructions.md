...
- Use the latest stable unit testing package whenever adding or updating unit test dependencies.
...
- Remove redundant parentheses unless they improve clarity.
- Use expression-bodied methods when possible.
- Use simplified collection initialization when possible.
- Make anonymous functions static when possible.
- Use pattern matching and switch expressions wherever possible.
- Use `nameof` instead of string literals when referring to member names.
- Prefer `TryGetValue` over a Dictionary indexer access guarded by a `ContainsKey` check to avoid double lookup.
...
- Apply all code style and formatting rules to test code as well as implementation code.
